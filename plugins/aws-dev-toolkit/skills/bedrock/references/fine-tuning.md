...
- **Provisioned Throughput is legacy and capacity-constrained.** PT is being superseded by the *reserved tier*, and the newest models (e.g. Anthropic Opus/Sonnet/Haiku 4.x) are **not** offered on PT at all. Internal guidance is to block PT at the org level and treat requests as rare exceptions — most "we need PT" asks are better served by on-demand custom deployment, reserved tier, or SageMaker inference. Don't architect around PT for a new build.
...
- **Designing a new build around Provisioned Throughput** — PT is legacy/capacity-constrained and absent on the newest models; prefer on-demand custom deployment, reserved tier, or SageMaker.
