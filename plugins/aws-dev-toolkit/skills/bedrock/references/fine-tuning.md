...
- **Don't default a new build to Provisioned Throughput.** For a customized model, **on-demand custom deployment** (pay-per-token, no pre-provisioned capacity) is the documented default for variable workloads — PT is the commit-to-capacity option, and not every model is offered on it (verify the [PT-supported models](https://docs.aws.amazon.com/bedrock/latest/userguide/prov-thru-supported.html) list for your target). Reach for on-demand custom deployment first, PT only when you have steady, high-volume traffic that justifies a capacity commitment, and SageMaker inference when you need GPU control or volumes Bedrock can't serve.
- **Prefer Reserved Tier over PT where the model offers it — but it's gated by per-model availability.** They reserve capacity on different infrastructures: **PT = capacity on the traditional Bedrock infrastructure; Reserved Tier = capacity on the Mantle infrastructure.** Reserved Tier is the better experience and capability set, so lean toward it when your target model is served on Mantle. The catch is that they're usually *not* both available for the same model — a model may be on one, the other, both, or neither (no capacity reservation possible). So: default to Reserved Tier when it's an option for your model, fall back to PT when it isn't, and always confirm the per-model matrix against current AWS docs rather than assuming.
- **For custom models, Bring Your Own Model (BYOM) is the path on Mantle.** BYOM is the Mantle successor to Custom Model Import — it lets you bring a customized model onto Mantle infrastructure. When a customer is planning custom-model deployments, factor in which path their target model supports (native FT, CMI, or BYOM) and verify current region/architecture support before committing a design.
...
- **Designing a new build around Provisioned Throughput** — PT is capacity-constrained and absent on the newest models; prefer on-demand custom deployment first, then Reserved Tier (where the model is served on Mantle), or SageMaker. Don't tell a customer PT is deprecated — there's no published deprecation notice.
- **Assuming PT and Reserved Tier are always both on the table** — RT is preferable where available, but the two reserve capacity on different infrastructures (PT = traditional Bedrock, RT = Mantle) and often aren't both offered for the same model; confirm the target model's availability matrix before promising RT.
